 ...
 *Place - Suite 330, Boston, MA 02111-1307, USA. You can also find it on the GNU
 *web site < https://www.gnu.org/licenses/gpl-3.0.en.html >
 **************************************************************************************************/
 ...
 * The total langth of the progress bar.
 */
static inline void printProgressBar(const size_t &current, const size_t &total,
                                    const size_t barLength = 50) {
  if (std::getenv("ULTRALISER_NO_PROGRESS"))
    return;
 ...
 * Prints a very simple progress bar in a loop that only increments evey 10% of
 * the loop.
 *
 * @param current
 * Current count
...
#define LOOP_PROGRESS_FRACTION(PROGRESS, TOTAL)                                \
  printFractionProgressBar(PROGRESS, TOTAL)
